...
export function buildMatcherRowHTML(m: MatcherRowOptions): string {
  ...
  return `
    <div class="group flex gap-2 items-center p-3 rounded-lg bg-white/3 hover:bg-white/5 transition-colors" data-mid="${m.id}">
      <input
        class="flex-[2] rounded-md bg-white/5 px-3 py-2 text-sm text-text outline-1 -outline-offset-1 outline-gray-700 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-blue-500"
        data-role="urlFilter"
        placeholder="Leave empty for all domains"
        value="${escapeHtml(displayUrlFilter)}"
      />
      <select
        class="flex-1 rounded-md bg-white/5 px-3 py-2 text-sm text-text outline-1 -outline-offset-1 outline-gray-700 focus:outline-2 focus:-outline-offset-2 focus:outline-blue-500 cursor-pointer"
        data-role="types"
      >
